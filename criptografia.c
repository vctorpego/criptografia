...
char *chinaDividida(char *string)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{ // conta a quantidade de letras 'c'
			cont_c++;
		}
		else if (string[i] == 't' || string[i] == 'T')
		{ // conta a quantidade de letras 't'
			cont_t++;
		}
	}
...
char *espelhoTai(char *string)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{ // somar as letras "a" e "z" e subtrair a posição do vetor (string[i])
			plot[i] = 'a' + 'z' - string[i];
		}
		...
		{ // os caracteres que não são letras permanecem inalterados.
			plot[i] = string[i];
		}
	}
...
char *criptografarDados(char *s)
{
	...
	do
	{
		...
		if (s[i] == ' ')
		{				// conta quantos espaços em branco tem e os acumula na variável cont_esp
